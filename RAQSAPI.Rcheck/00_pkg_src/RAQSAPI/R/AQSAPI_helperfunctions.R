...
#' @title title aqs_pad_variables_for_api
#' @description a helper function that pads the variables being sent
#'                to the API to ensure that the proper number of
#'                character digits are being sent to the the API.
#'                This function is not intended for use by end users.
#' @param variables
#' @return
#' @noRd
pad_variables_for_api <- function(variables)
{
  browser()
}
                    ...
                    user_key = NA, variables = NULL)
{
  ...
  variables %<>% pad_variables_for_api()
