...
RAQSAPI_signup_and_credentials_functions <- c("aqs_credentials", "aqs_sign_up") %>%
  sort() %>%
  tibble() %>%
  rename(FUN = ".") %>%
  mutate(PATH = glue::glue("../man/{FUN}"))

RAQSAPI_metadata_functions <- c("aqs_fields_by_service", "aqs_isavailable", "aqs_knownissues", "aqs_revisionhistory") %>%
  sort() %>%
  tibble() %>%
  rename(FUN = ".") %>%
  mutate(PATH = glue::glue("../man/{FUN}"))
...
ignoredfiles <- list("aqs_services_by_site", "aqs_services_by_county", "deprecated", "RAQSAPI", "aqs_metadata_service")
...
RAQSAPI_functions_table <- function()
{
  ...
  functiontable$functionfamily[which(functiontable$functionnames %in% setupfunctions)] <- "RAQSAPI setup
                                                            functions"
  functiontable$functionfamily[which(functiontable$functionnames %in% serviceshelperfunctions)] <- "RAQSAPI
                                                                   services
                                                                   functions"
